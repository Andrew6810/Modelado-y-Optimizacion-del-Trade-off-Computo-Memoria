...
- Genera 2 CSVs:
  1) curvas_amd_{arch}_bs{bs}.csv  -> train/val accuracy por epoch
  2) entrenamiento_amd_{arch}_bs{bs}.csv -> resultados finales en best_epoch
- NO hace profiling (eso lo hace profiling_mlp_2.py).

Uso:
    python entrenamiento_mlp_amd.py --batch_size 16 --arch MLP-3
"""
...
CURVAS_CSV = os.path.join(BASE_DIR, f"curvas_amd_{ARCH_SAFE}_bs{BATCH_SIZE_TARGET}.csv")
...
# Checkpoints .pt
CKPT_DIR = os.path.join(BASE_DIR, f"ckpts_train_amd_{ARCH_SAFE}_bs{BATCH_SIZE_TARGET}")
