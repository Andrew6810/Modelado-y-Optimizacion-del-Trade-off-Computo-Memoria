"""
entrenamiento_nvidia.py
=======================
Script 1 de 2: SOLO ENTRENAMIENTO (GPU NVIDIA).
- Busca el mejor epoch (1..10) para cada modelo baseline (normal).
- Entrena TODOS los modelos: BASELINE (normal) + PER-LAYER CHECKPOINTING
...
Estrategias de checkpointing utilizadas (catalogo combinado):
  ...
  - per-layer: torch.utils.checkpoint.checkpoint sobre capas/bloques
    individuales. Incluye configuraciones individuales, progresivas,
    conv/block + fc y full (identicas a los scripts de la GPU AMD,
    para que los CSV sean comparables fila a fila entre ambas GPUs).
  - uniform_seg{N}: torch.utils.checkpoint.checkpoint_sequential con N
    segmentos uniformes sobre las capas/bloques convolucionales.

La columna num_segments del CSV queda vacia para baseline y per-layer,
y contiene N para los modos uniform_seg{N}.

Uso:
    python entrenamiento_nvidia.py --batch_size 16 --arch LeNet-5
"""
...
# =============================================================
#   CAMPOS CSV (incluye num_segments para uniform segment ckpt)
# =============================================================
CURVAS_FIELDS = [
...
# ##############################################################
#   FASE 1: ENTRENAR TODOS LOS MODELOS (normal + uniform segment)
...
# Primero normales, luego modelos con checkpointing (per-layer + uniform segment)
for fase_normal in [True, False]:
    label = "NORMALES (baseline)" if fase_normal else "CHECKPOINTING (per-layer + uniform segment)"
    print(f"\n--- {label} ---")
