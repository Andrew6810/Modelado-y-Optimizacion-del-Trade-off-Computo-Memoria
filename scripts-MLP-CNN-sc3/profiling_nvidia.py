...
profiling_nvidia.py
===================
Script 2 de 2: SOLO PROFILING (GPU NVIDIA).
- Lee el best_epoch del CSV de entrenamiento (generado por entrenamiento_nvidia.py).
- Para cada modelo (baseline + per-layer ckpt + uniform segment ckpt),
  construye la red y hace profiling con datos sinteticos para medir:
  peak_memory, avg_memory, throughput, time_per_iter, total_flops,
  factor_recompute, ahorro_memoria.
- Genera: profiling_nvidia_{arch}_bs{bs}.csv
- NO entrena. Solo mide rendimiento.

Estrategias de checkpointing utilizadas (catalogo combinado):
  - normal (baseline, sin checkpoint)
  - per-layer: torch.utils.checkpoint.checkpoint sobre capas/bloques
    individuales (individuales, progresivas, conv/block + fc, full).
  - uniform_seg{N}: torch.utils.checkpoint.checkpoint_sequential.

Las configuraciones son identicas a los scripts de la GPU AMD, por lo
que los CSV de NVIDIA son comparables fila a fila con los de AMD.

Uso:
    python profiling_nvidia.py --batch_size 16 --arch LeNet-5

IMPORTANTE: Ejecutar DESPUES de entrenamiento_nvidia.py
"""
...
# CSV de entrada (generado por entrenamiento_nvidia.py)
...
# =============================================================
#   SPECS DE LA GPU NVIDIA
#   (Ajusta estos valores si conoces el modelo exacto. El job usa
#    partition=gpu_titan, nodelist=felix; valores base por defecto
#    para una NVIDIA TITAN/Tesla en cluster de investigacion.)
# =============================================================
GPU_SPECS = {
    "gpu_name":           "NVIDIA TITAN",
    "gpu_memory_gb":      12,
    "gpu_architecture":   "Volta/Turing/Ampere",
    "cuda_cores":         "N/A",
    "compute_units":      "N/A",
    "tensor_cores":       "N/A",
...
# Intentar detectar specs reales de la GPU en runtime
if torch.cuda.is_available():
    try:
        props = torch.cuda.get_device_properties(0)
        GPU_SPECS["gpu_name"] = props.name
        GPU_SPECS["gpu_memory_gb"] = round(props.total_memory / (1024**3), 2)
        GPU_SPECS["compute_capability"] = f"{props.major}.{props.minor}"
        GPU_SPECS["compute_units"] = props.multi_processor_count
        if hasattr(torch.version, "cuda") and torch.version.cuda:
            GPU_SPECS["cuda_version"] = torch.version.cuda
    except Exception as _e:
        print(f"[WARN] No se pudieron leer specs de GPU: {_e}")
...
def guardar_profiling(fila: dict):
    ...
    fila_completa.setdefault("num_segments", "")
...
if device == "cuda":
    ...
    print(f"[INFO] GPU memory   : {GPU_SPECS['gpu_memory_gb']} GB")
    print(f"[INFO] Compute cap  : {GPU_SPECS['compute_capability']}")
    print(f"[INFO] CUDA version : {GPU_SPECS['cuda_version']}")
...
# #############################################################
#   MODELOS CNN
#   Soportan DOS estrategias de activation checkpointing:
#     1) per-layer  : torch.utils.checkpoint.checkpoint sobre capas/
#        bloques individuales (selectivo, granular).
#     2) uniform_seg: torch.utils.checkpoint.checkpoint_sequential con
#        N segmentos uniformes sobre las capas/bloques convolucionales.
#   Ambas estrategias son mutuamente excluyentes por modelo: si
#   uniform_segments > 0 se ignora checkpoint_layers/block_ckpt.
# #############################################################
...
class LeNet5(nn.Module):
    def __init__(self, conv_channels=(6, 12), fc_units=(100, 64),
        ...
        # Secuencia para uniform segment checkpointing (2 sub-bloques conv)
    ...
    def forward(self, x):
        if self.uniform_segments > 0:
            # Estrategia uniform segment checkpointing
            x = checkpoint_sequential(self._uniform_seq,
                                      self.uniform_segments, x,
                                      use_reentrant=False)
        else:
            # Estrategia per-layer checkpointing (selectiva)
...
class VGG16Ckpt(nn.Module):
    def __init__(self, filtros=(24, 48, 96, 192, 192), capas_fc=(1536, 1536),
        ...
        self.fc2 = nn.Linear(capas_fc[0], capas_fc[1])
        self.fc3 = nn.Linear(capas_fc[1], 10)
        # Secuencia para uniform segment checkpointing (5 bloques)
    ...
    def forward(self, x):
        if self.uniform_segments > 0:
            ...
            # Estrategia per-layer checkpointing (selectiva por bloque)
...
#   Catalogo combinado:
#     - normal (baseline)
#     - per-layer: capas/bloques individuales, progresivos, conv+fc, full
#     - uniform_seg{N}: checkpoint_sequential con N segmentos
#
#   Tupla de modelo:
...
def gen_lenet5_models():
    ...
    # --- uniform segment checkpointing (LeNet-5 = 2 sub-bloques conv) ---
    uniform_configs = [
        ("uniform_seg2", "uniform_seg2", 2),
    ]
    models = []
    for act in ACTIVATIONS:
        for ckpt_dict, mode, capa in ckpt_configs:
            models.append(("LeNet-5", act, mode, capa, "conv(6, 12)_fc(100, 64)",
                           (6, 12), (100, 64), ckpt_dict, None, False, 0))
        for mode, capa, useg in uniform_configs:
            models.append(("LeNet-5", act, mode, capa, "conv(6, 12)_fc(100, 64)",
                           (6, 12), (100, 64), None, None, False, useg))
    return models
...
def gen_alexnet_models():
    ...
    # --- uniform segment checkpointing (AlexNet = 5 sub-bloques conv) ---
    uniform_configs = [
        ("uniform_seg2", "uniform_seg2", 2),
        ("uniform_seg3", "uniform_seg3", 3),
    ]
    models = []
    for act in ACTIVATIONS:
        for ckpt_dict, mode, capa in configs:
            models.append(("AlexNet", act, mode, capa, f"filt{filtros}_fc{capas_fc}",
                           filtros, capas_fc, ckpt_dict, None, False, 0))
        for mode, capa, useg in uniform_configs:
            models.append(("AlexNet", act, mode, capa, f"filt{filtros}_fc{capas_fc}",
                           filtros, capas_fc, None, None, False, useg))
    return models
...
def gen_vgg16_models():
    ...
    # --- uniform segment checkpointing (VGG-16 = 5 bloques conv) ---
    uniform_configs = [
        ("uniform_seg2", "uniform_seg2", 2),
        ("uniform_seg3", "uniform_seg3", 3),
    ]
    ...
    for act in ACTIVATIONS:
        for block_dict, fc_ck, mode, capa in configs:
            models.append(("VGG-16", act, mode, capa, f"filt{filtros}_fc{capas_fc}",
                           filtros, capas_fc, None, block_dict, fc_ck, 0))
        for mode, capa, useg in uniform_configs:
            models.append(("VGG-16", act, mode, capa, f"filt{filtros}_fc{capas_fc}",
                           filtros, capas_fc, None, None, False, useg))
    return models
...
def build_model(arch, act, conv_cfg, fc_cfg, ckpt_dict=None,
                block_ckpt=None, fc_block_ckpt=False, uniform_segments=0):
    if arch == "LeNet-5":
...
if not os.path.isfile(TRAIN_CSV):
    ...
    print(f"[ERROR] Ejecuta primero: python entrenamiento_nvidia.py --batch_size {batch} --arch {ARCH_TARGET}")
    sys.exit(1)

print(f"\n{'='*60}")
print(f"PROFILING: {ARCH_TARGET} | BATCH SIZE = {batch}")
print(f"Modelos totales: {len(all_models)} (baseline + per-layer ckpt + uniform segment ckpt)")
print(f"{'='*60}")
...
print(f"\n--- PROFILING: Modelos NORMALES (baseline) ---")
for tup in all_models:
    ...
    set_seed(42)
    model_obj = build_model(ARCH_TARGET, act, conv_cfg, fc_cfg,
                            ckpt_dict=ckpt_dict, block_ckpt=block_ckpt,
                            fc_block_ckpt=fc_ck, uniform_segments=useg).to(device)
...
# FASE 2: PROFILING CON CHECKPOINTING (per-layer + uniform segment)
print(f"\n--- PROFILING: Modelos con CHECKPOINTING (per-layer + uniform segment) ---")
for tup in all_models:
    model_name, act, mode, capa, config_str, conv_cfg, fc_cfg, ckpt_dict, block_ckpt, fc_ck, useg = tup
    if mode == "normal": continue
