...
- Para cada modelo (normal + ckpt + uniform segment ckpt), construye la red,
  y hace profiling con datos sinteticos para medir: peak_memory, avg_memory,
  throughput, time_per_iter, total_flops, factor_recompute, ahorro_memoria.
...
# CSV de entrada (generado por entrenamiento_cnn_sc3.py)
TRAIN_CSV = os.path.join(BASE_DIR, f"entrenamiento_{ARCH_SAFE}_bs{BATCH_SIZE_TARGET}.csv")

# CSV de salida de profiling
PROF_CSV = os.path.join(BASE_DIR, f"profiling_{ARCH_SAFE}_bs{BATCH_SIZE_TARGET}.csv")
...
PROF_FIELDS = [
    ...
    "num_segments",
    # GPU specs
    "gpu_name", "gpu_memory_gb", "gpu_mem_limit_gb", "gpu_architecture", "cuda_cores",
...
print(f"\n{'='*60}")
print(f"PROFILING: {ARCH_TARGET} | BATCH SIZE = {batch} | GPU MEM = {GPU_MEM_LIMIT_GB} GB")
...
# FASE 2: PROFILING CON CHECKPOINT (incluye uniform segment)
print(f"\n--- PROFILING: Modelos con CHECKPOINT (incl. uniform segment) ---")
for tup in all_models:
    model_name, act, mode, capa, config_str, conv_cfg, fc_cfg, ckpt_dict, block_ckpt, fc_ck, useg = tup
    if mode == "normal": continue
